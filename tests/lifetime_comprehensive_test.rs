...
// ========== 测试 lifetime=program + key=ptr (应该保留缓存) ==========
...
#[test]
fn test_lifetime_program_with_ptr_keeps_cache() {
    ...
    // 第二次调用相同数组 - lifetime=program + key=ptr 应该保留缓存
    let result2 = calc_ptr_program_start(&arr);
    assert_eq!(result2, 15);
    assert_eq!(PTR_PROGRAM_CALL_COUNT.load(Ordering::SeqCst), 1, "program+ptr 模式应该保留缓存");
    
    // 清除缓存后重新计算
    calc_ptr_program_clear();
    let result3 = calc_ptr_program_start(&arr);
    assert_eq!(result3, 15);
    assert_eq!(PTR_PROGRAM_CALL_COUNT.load(Ordering::SeqCst), 2, "清除后应该重新计算");
}
...
fn test_lifetime_program_ptr_different_arrays() {
    ...
    assert_eq!(result3, 6);
    assert_eq!(PTR_PROGRAM_CALL_COUNT.load(Ordering::SeqCst), 2, "相同地址应该命中缓存");
}

// ========== 测试 lifetime=program + key=ref (应该保留缓存) ==========
...
#[test]
fn test_lifetime_program_with_ref_keeps_cache() {
    ...
    // 第二次调用相同数组 - lifetime=program + key=ref 应该保留缓存
    let result2 = calc_ref_program_start(&arr);
    assert_eq!(result2, 15);
    assert_eq!(REF_PROGRAM_CALL_COUNT.load(Ordering::SeqCst), 1, "program+ref 模式应该保留缓存");
}
...
fn test_lifetime_program_ref_different_arrays_same_content() {
    ...
    // 调用 arr2 - ref 模式先比地址，再比内容，内容相同应该命中缓存
    let result2 = calc_ref_program_start(&arr2);
    assert_eq!(result2, 6);
    assert_eq!(REF_PROGRAM_CALL_COUNT.load(Ordering::SeqCst), 1, "ref 模式内容相同应该命中缓存");
}

// ========== 测试 lifetime=program + key=val (应该清除缓存) ==========
...
#[test]
fn test_lifetime_program_with_val_clears_cache() {
    ...
    // 第二次调用 - 即使是 lifetime=program，val 模式也应该清除缓存
    // 因为键中不包含地址信息，不同问题的数组如果值相同会错误命中缓存
    let result2 = calc_val_program_start(&arr);
    assert_eq!(result2, 15);
    assert_eq!(VAL_PROGRAM_CALL_COUNT.load(Ordering::SeqCst), 2, 
        "val 模式即使设置 program 也应该清除缓存（键中不包含地址）");
}
...
fn test_multithread_lifetime_program() {
    ...
    assert_eq!(result2, 100);
    assert_eq!(MULTI_PROGRAM_CALL_COUNT.load(Ordering::SeqCst), 2, "无引用参数时，即使多线程 program+ptr 也应该清除缓存");
}
...
fn test_manual_cache_clear() {
    ...
    assert_eq!(result2, 25);
    assert_eq!(MANUAL_CLEAR_CALL_COUNT.load(Ordering::SeqCst), 2, "无引用参数时应该清除缓存");
    
    // 即使手动调用 clear 也不会有影响（因为每次都清除）
    calc_manual_clear_clear();
    
    // 第三次调用 - 仍然重新计算
    let result3 = calc_manual_clear_start(5);
    assert_eq!(result3, 25);
    assert_eq!(MANUAL_CLEAR_CALL_COUNT.load(Ordering::SeqCst), 3);
}
...
// 没有引用参数的函数，即使设置 lifetime=program，也应该清除缓存
// 因为键中不包含地址信息
#[memo(key=ptr, lifetime=program)]
...
#[test]
fn test_lifetime_program_no_ref_params_always_clears() {
    // 测试 key=ptr + 无引用参数
    ...
    assert_eq!(result2, 25);
    assert_eq!(NO_REF_PROGRAM_PTR_CALL_COUNT.load(Ordering::SeqCst), 2, 
        "无引用参数时，即使 key=ptr + lifetime=program 也应该清除缓存");
    ...
    assert_eq!(NO_REF_PROGRAM_REF_CALL_COUNT.load(Ordering::SeqCst), 2, 
        "无引用参数时，即使 key=ref + lifetime=program 也应该清除缓存");
    ...
    assert_eq!(NO_REF_PROGRAM_VAL_CALL_COUNT.load(Ordering::SeqCst), 2, 
        "无引用参数时，key=val + lifetime=program 应该清除缓存");
}
...
fn test_lifetime_combinations() {
    ...
    f4_start(5);
    f4_start(5);
    assert_eq!(COUNT4.load(Ordering::SeqCst), 2, "无引用参数时，program+ptr 应该清除");
    ...
    f5_start(5);
    f5_start(5);
    assert_eq!(COUNT5.load(Ordering::SeqCst), 2, "无引用参数时，program+ref 应该清除");
    ...
    f6_start(5);
    f6_start(5);
    assert_eq!(COUNT6.load(Ordering::SeqCst), 2, "program+val 应该清除（键中无地址）");
}
