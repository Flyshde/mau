...
**支持的语法**：
- 范围表达式：`each!(|i| { ... }, 0..data.len())`
- 闭区间：`each!(|i| { ... }, 0..=5)`
- 部分范围：`each!(|i| { ... }, 2..5)`
- 任何迭代器：`each!(|x| { ... }, data.iter())`

## 更新日志

### v0.1.16 (最新)
- ✨ **新增 `each!` 宏**：对指定范围内的每个索引执行闭包
  - 语法：`each!(|i| { statements }, 0..data.len())`
  - 等价于 `for i in 0..data.len() { statements }`
  - 支持范围表达式和迭代器
- 🔧 **ptr/ref 模式智能缓存清除**：使用 `ptr` 或 `ref` 模式时，通过 `solve!` 调用会自动清除缓存
  - 避免内存泄漏：地址模式下的缓存无法跨问题复用
  - 提升性能：减少无效缓存占用内存
- 📝 **更新文档**：添加 `each!` 宏和缓存清除策略说明
